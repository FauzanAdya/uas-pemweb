...
<div class="flex-grow-1 p-4">
    ...
    <form method="GET" class="d-flex gap-2 mb-4">
        ...
        <button type="submit" class="btn text-white" style="background:#3B4A1F">Filter</button>
    </form>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="card p-3 border-success">
                ...
                <div class="fs-4 fw-bold text-success">Rp <?= number_format($totalPemasukan,0,',','.') ?></div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card p-3 border-danger">
                <div class="small text-muted">Total Pengeluaran</div>
                <div class="fs-4 fw-bold text-danger">Rp <?= number_format($totalPengeluaran,0,',','.') ?></div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card p-3" style="border-color:#3B4A1F">
                <div class="small text-muted">Keuntungan Bersih</div>
                <div class="fs-4 fw-bold" style="color:#3B4A1F">Rp <?= number_format($keuntungan,0,',','.') ?></div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header fw-bold text-success">Pemasukan (Pesanan Selesai)</div>
                <div class="card-body p-0">
                    <table class="table table-sm mb-0">
                        <thead class="table-light"><tr><th>Nama</th><th>Total</th><th>Tgl</th></tr></thead>
                        <tbody>
                        ...
                        <?php if (empty($transaksiMasuk)): ?>
                            <tr><td colspan="3" class="text-center text-muted">Belum ada pemasukan</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header fw-bold text-danger d-flex justify-content-between">
                    ...
                    <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#modalPengeluaran">+ Catat</button>
                </div>
                <div class="card-body p-0">
                    <table class="table table-sm mb-0">
                        <thead class="table-light"><tr><th>Keterangan</th><th>Jumlah</th><th>Tgl</th><th></th></tr></thead>
                        <tbody>
                        <?php foreach ($transaksiKeluar as $t): ?>
                            <tr>
                                ...
                                <td><a href="index.php?page=keuangan&action=hapus&id=<?= $t['id'] ?>" class="text-danger small" onclick="return confirm('Hapus?')">Hapus</a></td>
                            </tr>
                        <?php endforeach; ?>
                        <?php if (empty($transaksiKeluar)): ?>
                            <tr><td colspan="4" class="text-center text-muted">Belum ada pengeluaran</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
...
<div class="modal fade" id="modalPengeluaran" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST" action="index.php?page=keuangan&action=tambah_pengeluaran">
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Keterangan</label>
                        <input type="text" name="keterangan" class="form-control" placeholder="cth: Beli kertas wrap" required>
                    </div>
                    <div class="mb-3">
