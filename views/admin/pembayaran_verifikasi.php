            ...
            <div class="card p-3 h-100">
                ...
                <table class="table table-borderless table-sm mb-0">
                    <tr>
                        ...
                        <td class="fw-bold"><?= htmlspecialchars($pembayaran['nama_pemesan']) ?></td>
                    </tr>
                    <tr>
                        <td class="text-muted">No WA</td>
                        <td><a href="https://wa.me/<?= $pembayaran['no_wa'] ?>" target="_blank"><?= $pembayaran['no_wa'] ?></a></td>
                    </tr>
                    <tr>
                        <td class="text-muted">Total Harga</td>
                        <td class="fw-bold">Rp <?= number_format($pembayaran['total_harga'], 0, ',', '.') ?></td>
                    </tr>
                    <tr>
                        <td class="text-muted">Tgl Ambil</td>
                        <td><?= date('d/m/Y', strtotime($pembayaran['tanggal_ambil'])) ?></td>
                    </tr>
                    <tr>
                        <td class="text-muted">Tipe Bayar</td>
                        <td>
                            <?php if ($isLunas): ?>
            ...
            <div class="card p-3">
                ...
                <form method="POST" action="index.php?page=pembayaran&action=konfirmasi" class="mb-2">
                    ...
                    <button type="submit" class="btn btn-success w-100 fw-bold py-2"
                        onclick="return confirm('Konfirmasi bukti pembayaran <?= $isLunas ? 'LUNAS' : 'DP' ?> ini valid?')">
                        ✓ Tandai <?= $isLunas ? ' LUNAS DITERIMA' : ' DP DITERIMA' ?>
                    </button>
                </form>

                <!-- Tombol WA pembayaran diterima -->
