...
<div class="flex-grow-1 p-4">
    ...
    <?php
    /** @var array $stok */  
    if (!empty($stokMenuipis)): ?>
    <div class="alert alert-warning">
        <strong>⚠ Peringatan!</strong> Stok bahan berikut hampir habis:
        <?php foreach($stokMenuipis as $s): ?>
            <span class="badge bg-warning text-dark ms-1"><?= $s['nama_bahan'] ?> (<?= $s['jumlah'] ?> <?= $s['satuan'] ?>)</span>
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
    <div class="card">
        <div class="card-body p-0">
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr><th>#</th><th>Nama Bahan</th><th>Jumlah</th><th>Satuan</th><th>Stok Minimum</th><th>Status</th><th>Aksi</th></tr>
                </thead>
                <tbody>
                ...
                <tr class="<?= $s['jumlah'] <= $s['stok_minimum'] ? 'table-warning' : '' ?>">
                    ...
                    <td>
                        <a href="index.php?page=stok&action=edit&id=<?= $s['id'] ?>" class="btn btn-sm btn-outline-primary">Edit</a>
                        <a href="index.php?page=stok&action=hapus&id=<?= $s['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus bahan ini?')">Hapus</a>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php if (empty($stok)): ?>
                    <tr><td colspan="7" class="text-center text-muted py-3">Belum ada data stok bahan</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
