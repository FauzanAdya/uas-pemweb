...
class PesananModel {
    ...
    public function simpan($data) {
        $stmt = $this->db->prepare(
            "INSERT INTO pesanan 
            (nama_pemesan, no_wa, produk_id, jumlah, tanggal_pesan, tanggal_ambil, 
             ucapan, is_custom, warna_kertas, jenis_isi, tambahan, total_harga, 
             tipe_pengambilan, status, status_bayar) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'pending','belum_lunas')"
        );
        $stmt->bind_param(
            "ssiiisssssis s",
            $data['nama_pemesan'], $data['no_wa'], $data['produk_id'],
